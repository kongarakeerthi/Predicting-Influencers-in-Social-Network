...
def mlModelsBestFeatures():
    ...
    #text.insert(END,"Removed features are: "+str(X_train_log.columns[index])+"\n")
...
def build(X_train_log,X_test_log,y_train,y_test):
    ...
    model_SVM = SVC(probability=True, C=10,gamma=0.000001)
    ...
    #MLP
    model_MLP = MLPClassifier(activation= 'tanh', solver= 'sgd')
...
ml= Button(main, text="Select Best Features", command=mlModelsBestFeatures)
