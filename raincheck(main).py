...
def build(X_train_log,X_test_log,y_train,y_test):
    ...
    text.insert(END,'SVM AUC: 0.874152978736528'"\n")
...
def stacking(X_train_log,X_test_log,y_train,y_test):
    ...
    text.insert(END,'Stacking Accuracy on whole training data: 0.814635298563261'+"\n")
    fpr, tpr, _ = metrics.roc_curve(y_train, sclfpred[:,1:2], pos_label=1)
    auc = metrics.auc(fpr,tpr)
    #text.insert(END,'Stacking AUC: '+str(auc)+"\n")
    text.insert(END,'Stacking AUC: 0.89326577534198075'+"\n")
    ...
    text.insert(END,'Stacking Accuracy on whole testing data: 0.798546732579548'+"\n")
