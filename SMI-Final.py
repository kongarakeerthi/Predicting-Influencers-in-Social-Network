...
def ok():
     global X_train_log,X_test_log,y_test,y_train
     stacking(X_train_log,X_test_log,y_train,y_test)
...
def mlModelsBestFeatures():
    ...
    cols = ['A_following_count','B_following_count', 'B_network_feature_2', 'A_retweets_sent', 'A_network_feature_2', 'A_is_popular']
    ...
    text.insert(END,"Total No of Selected Features: "+str(Xtrainlog.shape[1])+"\n")
...
def build(X_train_log,X_test_log,y_train,y_test):
    ...
    model_SVM = SVC(probability=True, C=100, gamma=0.001)
...
def stacking(X_train_log,X_test_log,y_train,y_test):
    ...
    clf1 = GaussianNB()
    clf4 = RandomForestClassifier(n_estimators = 100,max_depth=5)
    clf5 = LogisticRegression()
    nb=GaussianNB()
    sclf = StackingCVClassifier(classifiers=[clf2, clf3,clf4],
                              shuffle = False,
                              use_probas = True,
                              cv = 10,
                              n_jobs = -1,
...
def uploadPred():
    ...
    if l[2] > l[5]:
        ...
        #l= predictable.tolist()
    #text.insert(END, l.count(0)+"---------------------"+l.count(1))
...
ml= Button(main, text="Stacking", command=ok)
